...
void VecGaussIntegrationPhi23_PhiLCBar_PhiLSBar(vector <double> &AResults, vector <double> &BResults, int nR1, int nR2Leg, int nR2Lag, int nR3Leg, int nR3Lag, int nR12, int nR13, int nPhi23, double CuspR2, double CuspR3, double kappa, double mu, int sf, int NumPowers, vector <rPowers> &Powers, int Omega)
{
	...
	for (int i = 0; i < nR1; i++) {  // r1 integration
		vector <long double> TempAResults(2*NumPowers, 0.0), TempBResults(2*NumPowers, 0.0);
		...
		CreateRPowerLUT(r1Pow, r1, Omega+1);
		...
		// Do Gauss-Legendre for r2 integration over [0,r1] and then Gauss-Laguerre over [r1,inf).
			...
			for (int m = 0; m < nR2Leg; m++) {
				r2Weights[m] = LegendreWeightsR2[m] * expl(-Powers[0].beta*r2Abscissas[m]) * (r1-0.0)/2.0;
			}
			for (int m = nR2Leg; m < NumR2Points; m++) {
				r2Abscissas[m] = (LaguerreAbscissasR2[m-nR2Leg] + r1*Powers[0].beta) / Powers[0].beta;
				r2Weights[m] = LaguerreWeightsR2[m-nR2Leg] * expl(-Powers[0].beta*r1) / Powers[0].beta;
			}
		}
		...
		for (int j = 0; j < NumR2Points; j++) {  // r2 integration
			...
			CreateRPowerLUT(r2Pow, r2, Omega+1);
			...
			// Do Gauss-Legendre for r3 integration over [0,r1] and then Gauss-Laguerre over [r1,inf).
				...
				for (int m = 0; m < nR3Leg; m++) {
					r3Weights[m] = LegendreWeightsR3[m] * expl(-Powers[0].gamma*r3Abscissas[m]) * (r1-0.0)/2.0;
				}
			...
			for (int g = 0; g < NumR3Points; g++) {  // r3 integration
				...
				long double Coeff = r3Weights[g] * r2Weights[j] * r1Weights[i] * SqrtKappa * 0.70710678118654752440 / 2.0 * PI/nPhi23;
				...
				for (int k = 0; k < nR12; k++) {  // r12 integration
					...
					long double Sin12 = sqrtl(1.0 - Cos12*Cos12);
					long double rho = 0.5 * sqrtl(2.0*(r1*r1 + r2*r2) - r12*r12);
					long double j1rho = sf_bessel_j1(kappa*rho);
					long double n1rho = sf_bessel_n1(kappa*rho);
					long double ExpR12R3 = expl(-(r12/2.0 + r3));
					...
					for (int p = 0; p < nR13; p++) {  // r13 integration
						...
						long double Sin13 = sqrtl(1.0 - Cos13*Cos13);
						long double rhop = 0.5 * sqrtl(2.0*(r1*r1 + r3*r3) - r13*r13);
						long double j1rhop = sf_bessel_j1(kappa*rhop);
						long double n1rhop = sf_bessel_n1(kappa*rhop);
						long double ExpR13R2 = expl(-(r13/2.0 + r2));
						...
						// Phi1LS part
						long double S22 = ExpR12R3 * j1rho;
						long double S23 = ExpR13R2 * j1rhop;
						long double AngPhi1S22 = (r1 + r2 * Cos12) / rho;
						long double AngPhi1S23 = (r1 + r3 * Cos13) / rhop;
						...
						long double fsh1rho = fshielding1(rho, mu) / rho * (n1rho + CosKRho) - 0.5 * fshielding2(rho, mu) * n1rho;
						long double fsh1rhop = fshielding1(rhop, mu) / rhop * (n1rhop + cosl(kappa*rhop)) - 0.5 * fshielding2(rhop, mu) * n1rhop;
						long double AngPhi1C22 = (r1 + r2 * Cos12) / rho;
						long double AngPhi1C23 = (r1 + r3 * Cos13) / rhop;
						long double fOuterC1 = -AngPhi1C22 * ExpR12R3 * (Pot * n1rho * fshrho + fsh1rho);
						fOuterC1 -= sf * AngPhi1C23 * ExpR13R2 * (PotP * n1rhop * fshrhop + fsh1rhop);
						// Phi2LC part
						//@TODO: Same question as above
						long double AngPhi2C22 = (r2 + r1 * Cos12) / rho;

						for (int m = 1; m <= nPhi23; m++) {  // phi_23 integration
							long double Phi23 = (2.0*m - 1.0)*PI/(2.0*nPhi23);
							long double r23 = sqrtl(r2*r2 + r3*r3 - 2.0*r2*r3*(Sin12*Sin13*cosl(Phi23) + Cos12*Cos13));
							long double Cos23 = (r2*r2 + r3*r3 - r23*r23) / (2.0*r2*r3);
							CreateRPowerLUT(r23Pow, r23, Omega);

							// Phi2LS part
							long double AngPhi2S23 = (r1 * Cos12 + r3 * Cos23) / rhop;
							long double fOuterS2 = AngPhi2S22 * Pot * S22 + sf * AngPhi2S23 * PotP * S23;

							// Phi2LC part
							long double AngPhi2C23 = (r1 * Cos12 + r3 * Cos23) / rhop;
							long double fOuterC2 = -AngPhi2C22 * ExpR12R3 * (Pot * n1rho * fshrho + fsh1rho);
							fOuterC2 -= sf * AngPhi2C23 * ExpR13R2 * (PotP * n1rhop * fshrhop + fsh1rhop);
	...
	for (int i = 0; i < nR1; i++) {  // r1 integration
		...
		for (int j = 0; j < NumR2Points; j++) {  // r2 integration
			...
			// Do Gauss-Legendre for r3 integration over [0,r2] and then Gauss-Laguerre over [r2,inf).
				...
				NumR3Points = nR3Leg + nR3Lag;
				for (int m = 0; m < nR3Leg; m++) {
					r3Weights[m] = LegendreWeightsR3[m] * expl(-Powers[0].gamma*r3Abscissas[m]) * (r2-0.0)/2.0;
				}
				for (int m = nR3Leg; m < NumR3Points; m++) {
					r3Abscissas[m] = (LaguerreAbscissasR3[m-nR3Leg] + r2*Powers[0].gamma) / Powers[0].gamma;
					r3Weights[m] = LaguerreWeightsR3[m-nR3Leg] * expl(-Powers[0].gamma*r2) / Powers[0].gamma;
				}
			}
			...
			for (int g = 0; g < NumR3Points; g++) {  // r3 integration
				...
				long double Coeff = r3Weights[g] * r2Weights[j] * r1Weights[i] * SqrtKappa * 0.70710678118654752440 / 2.0 * PI/nPhi13;
				CreateRPowerLUT(r3Pow, r3, Omega);
				...
				for (int k = 0; k < nR23; k++) {  // r23 integration
					...
					CreateRPowerLUT(r23Pow, r23, Omega);
					long double Cos23 = (r2*r2 + r3*r3 - r23*r23) / (2.0*r2*r3);
					long double Sin23 = sqrtl(1.0 - Cos23*Cos23);
					...
					for (int p = 0; p < nR12; p++) {  // r12 integration
						...
						long double fshrho = fshielding(rho, mu);
						long double AngPhi1C22 = (r1 + r2 * Cos12) / rho;
						long double fOuterC1Part = -AngPhi1C22 * ExpR12R3 * (Pot * n1rho * fshrho);
						// Phi2LC
						long double AngPhi2C22 = (r2 + r1 * Cos12) / rho;
						long double fOuterC2Part = -AngPhi2C22 * ExpR12R3 * (Pot * n1rho * fshrho);

						for (int m = 1; m <= nPhi13; m++) {  // phi_13 integration
							//long double Phi13 = (2.0*m - 1.0)*PI/(2.0*nPhi13);
							long double Phi13 = (2*m - 1)*PI/(2*nPhi13);
							//long double r13 = sqrtl(r1*r1 + r3*r3 - 2.0*r1*r3*(Sin12*Sin23*cosl(Phi13) + Cos12*Cos23));
							long double r13 = sqrtl(r1*r1 + r3*r3 - 2*r1*r3*(Sin12*Sin23*cosl(Phi13) + Cos12*Cos23));
							//long double Cos13 = (r1*r1 + r3*r3 - r13*r13) / (2.0*r1*r3);
							long double Cos13 = (r1*r1 + r3*r3 - r13*r13) / (2*r1*r3);
							CreateRPowerLUT(r13Pow, r13, Omega);
							//long double rhop = 0.5 * sqrtl(2.0*(r1*r1 + r3*r3) - r13*r13);
							...
							long double fshrhop = fshielding(rhop, mu);
							long double AngPhi1C23 = (r1 + r3 * Cos13) / rhop;
							long double fOuterC1 = fOuterC1Part - sf * AngPhi1C23 * ExpR13R2 * (PotP * n1rhop * fshrhop);

							// Phi2LC part
							//@TODO: Same question as above
							long double AngPhi2C23 = (r1 * Cos12 + r3 * Cos23) / rhop;
							long double fOuterC2 = fOuterC2Part - sf * AngPhi2C23 * ExpR13R2 * (PotP * n1rhop * fshrhop);
...
void VecGaussIntegrationPhi12_PhiLCBar_PhiLSBar_R23Term(vector <double> &AResults, vector <double> &BResults, int nR1, int nR2Leg, int nR2Lag, int nR3Leg, int nR3Lag, int nPhi12, int nR13, int nR23, double CuspR2, double CuspR3, double kappa, double mu, int sf, int NumPowers, vector <rPowers> &Powers, int Omega)
{
	vector <long double> LaguerreAbscissasR2, LaguerreWeightsR2, LegendreAbscissasR2, LegendreWeightsR2;
	vector <long double> LaguerreAbscissasR3, LaguerreWeightsR3, LegendreAbscissasR3, LegendreWeightsR3;
	vector <long double> LegendreAbscissasR13, LegendreWeightsR13;
	...
	for (int i = 0; i < nR1; i++) {  // r1 integration
		...
		for (int j = 0; j < NumR3Points; j++) {  // r3 integration
			...
			long double a13 = fabs(r1-r3);
			long double b13 = fabs(r1+r3);
			CreateRPowerLUT(r3Pow, r3, Omega+1);
			ChangeOfIntervalNoResize(LegendreAbscissasR13, r13Array, a13, b13);
			...
			// Do Gauss-Legendre for r2 integration over [0,r3] and then Gauss-Laguerre over [r3,inf).
				...
				NumR2Points = nR2Leg + nR2Lag;
				for (int m = 0; m < nR2Leg; m++) {
					r2Weights[m] = LegendreWeightsR2[m] * expl(-Powers[0].beta*r2Abscissas[m]) * (r3-0.0)/2.0;
				}
				for (int m = nR2Leg; m < NumR2Points; m++) {
			...
			for (int g = 0; g < NumR2Points; g++) {  // r2 integration
				...
				long double a23 = fabs(r2-r3);
				long double b23 = fabs(r2+r3);
				long double Coeff = r3Weights[j] * r2Weights[g] * r1Weights[i] * SqrtKappa * 0.70710678118654752440 / 2.0 * PI/nPhi12;
				CreateRPowerLUT(r2Pow, r2, Omega+1);
				ChangeOfIntervalNoResize(LegendreAbscissasR23, r23Array, a23, b23);
			...
			for (int g = 0; g < NumR3Points; g++) {  // r3 integration
				...
				for (int k = 0; k < nR12; k++) {  // r12 integration
					...
					for (int p = 0; p < nR13; p++) {  // r13 integration
						...
						// Phi1LS part
						long double S22 = ExpR12R3 * j1rho;  // S22
						long double S23 = ExpR13R2 * j1rhop;  // S23
						long double AngPhi1S22 = (r1 + r2 * Cos12) / rho;
						long double AngPhi1S23 = (r1 + r3 * Cos13) / rhop;
						// Phi2LS part
						long double AngPhi2S22 = (r2 + r1 * Cos12) / rho;
						...
						for (int m = 1; m <= nPhi23; m++) {  // phi_23 integration
							...
							long double PotP = 2.0/r1 - 2.0/r3 - 2.0/r12 + 2.0/r23;
							long double Pot = 2.0/r1 - 2.0/r2 - 2.0/r13 + 2.0/r23;
