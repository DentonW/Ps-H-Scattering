...
class rPowers
{
	public:
		...
		int Index;
		friend std::ostream& operator << (std::ostream& os, const rPowers &rp)
		{
			os << rp.Index << " | " << rp.ki << " " << rp.li << " " << rp.mi << " " << rp.ni << " " << rp.pi << " " << rp.qi << " " << rp.alpha << " " << rp.beta << " " << rp.gamma;
			return os;
		}
};
...
bool	ReadShortHeader(ifstream &FileShortRange, int &Omega, int &IsTriplet, int &Ordering, int &NumShortTerms, double &Alpha, double &Beta, double &Gamma, int &l);
