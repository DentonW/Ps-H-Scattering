...
real*16 function HylleraasIntegral(RunCalc, j1, j2, j3, j12, j23, j31, alpha, beta, gamma, qmax, pmax, Method, WMatrix, CMatrix)
	use WCLimits
	implicit none
	common /globals/ PI
	logical RunCalc
	...
	do k12 = 0, int(L12), 1  !@TODO: Determine if L12, L23 and L31 can be real.
		do k31 = 0, int(L31), 1
			do k23 = 0, int(L23), 1
				...
				!Res1W = W(j1 + 2*q + 2*k12 + 2*k31 + 2, j2 + j12 - 2*k12 + 2*k23 + 2, j3 + j23 - 2*q - 2*k23 + j31 - 2*k31 + 2, alpha, beta, gamma, pmax)
				!Res2W = W(j1 + 2*q + 2*k12 + 2*k31 + 2, j3 + j31 - 2*k31 + 2*k23 + 2, j2 + j12 - 2*q - 2*k12 + j23 - 2*k23 + 2, alpha, gamma, beta, pmax)
				!Res3W = W(j2 + 2*q + 2*k12 + 2*k23 + 2, j1 + j12 - 2*k12 + 2*k31 + 2, j3 + j23 - 2*q - 2*k23 + j31 - 2*k31 + 2, beta, alpha, gamma, pmax)
				!Res4W = W(j2 + 2*q + 2*k12 + 2*k23 + 2, j3 + j23 - 2*k23 + 2*k31 + 2, j1 + j12 - 2*q - 2*k12 + j31 - 2*k31 + 2, beta, gamma, alpha, pmax)
				!Res5W = W(j3 + 2*q + 2*k23 + 2*k31 + 2, j1 + j31 - 2*k31 + 2*k12 + 2, j2 + j12 - 2*q - 2*k12 + j23 - 2*k23 + 2, gamma, alpha, beta, pmax)
				!Res6W = W(j3 + 2*q + 2*k23 + 2*k31 + 2, j2 + j23 - 2*k23 + 2*k12 + 2, j1 + j12 - 2*q - 2*k12 + j31 - 2*k31 + 2, gamma, beta, alpha, pmax)
...
!subroutine NewDgesv(n, a, b, Info)
!	implicit none
...
!
!	eps = 1q-11  !@TODO: This may need to be adjusted.
!	
