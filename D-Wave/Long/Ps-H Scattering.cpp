...
int main(int argc, char *argv[])
{
	...
	if (Node == 0) {
		...
		if (!ReadShortHeader(FileShortRange, Omega, IsTriplet, Ordering, NumShortTerms, Alpha, Beta, Gamma)) {
			cerr << argv[3] << " is not a valid Ps-H D-wave short-range file...exiting." << endl;
			FinishMPI();
			exit(3);
		}
		...
		if (IsTriplet == 0) {
			sf = 1;
			cout << "D-Wave Singlet Ps-H" << endl;
		}
		else if (IsTriplet == 1) {
			sf = -1;
			cout << "D-Wave Triplet Ps-H" << endl;
		}
		else {
	...
	if (Node == 0) {
		...
		// Create phi2 terms
		for (int i = 0; i < NumTermsQi0; i++) {
			PowerTableQi0[NumTermsQi0+i].ki = PowerTableQi0[i].ki - 2;
			PowerTableQi0[NumTermsQi0+i].li = PowerTableQi0[i].li + 2;
		...
		for (int i = 0; i < NumTermsQiGt0; i++) {
			PowerTableQiGt0[NumTermsQiGt0+i].ki = PowerTableQiGt0[i].ki - 2;
			PowerTableQiGt0[NumTermsQiGt0+i].li = PowerTableQiGt0[i].li + 2;
...
//#endif//USE_MPI
	...
	CalcARowAndBVector(Node, NumTermsQi0, NumTermsQiGt0, Omega, PowerTableQi0, PowerTableQiGt0, AResultsQi0, AResultsQiGt0, ARow, BResultsQi0, BResultsQiGt0, B, SLS, SLC, q, r2Cusp, r3Cusp, Alpha, Beta, Gamma, Kappa, Mu, Lambda1, Lambda2, Lambda3, ShPower, sf);
	//TimeEnd = time(NULL);
...
bool ReadShortHeader(ifstream &FileShortRange, int &Omega, int &IsTriplet, int &Ordering, int &NumShortTerms, double &Alpha, double &Beta, double &Gamma)
{
	...
	if (MagicNum != 0x31487350 || Version != 6 || HeaderLen != 80 || DataFormat != 8 || Omega < 0 || NumShortTerms1 < 1
		|| Alpha <= 0.0 || Beta <= 0.0 || Gamma <= 0.0 /*|| NumShortTerms1 != NumShortTerms2*/ || LValue != 2 || Formalism != 1 || NumSets != 1) {
		cout << Version << " " << HeaderLen << " " << DataFormat << " " << Omega << " " << NumShortTerms1 << " " << NumShortTerms2 << endl;
		return false;
	}
			  ...
			  double r3Cusp, double alpha, double beta, double gamma, double kappa, double mu, double lambda1, double lambda2, double lambda3, int shpower, int sf)
{
	...
	if (Node == 0) {
		...
		GaussIntegrationPhi23_LongLong(q.LongLong_r1, q.LongLong_r2Leg, q.LongLong_r2Lag, q.LongLong_r3Leg, q.LongLong_r3Lag, q.LongLong_r12, q.LongLong_r13, q.LongLong_phi23, r2Cusp, r3Cusp, kappa, mu, shpower, sf, ARow[0], SLC, B[0], SLS);
		...
		GaussIntegrationPhi13_LongLong_R23Term(q.LongLongr23_r1, q.LongLongr23_r2Leg, q.LongLongr23_r2Lag, q.LongLongr23_r3Leg, q.LongLongr23_r3Lag, q.LongLongr23_phi12, q.LongLongr23_r13, q.LongLongr23_r23, r2Cusp, r3Cusp, kappa, mu, shpower, sf, CLCTemp, SLCTemp, CLSTemp, SLSTemp);
	...
	if (NumTermsQi0 > 0) {  // Skips when no terms with qi == 0
		...
		VecGaussIntegrationPhi23_PhiLCBar_PhiLSBar(AResultsQi0, BResultsQi0, q.ShortLong_r1, q.ShortLong_r2Leg, q.ShortLong_r2Lag, q.ShortLong_r3Leg, q.ShortLong_r3Lag, q.ShortLong_r12, q.ShortLong_r13, q.ShortLong_phi23, r2Cusp, r3Cusp, kappa, mu, shpower, sf, NumTermsQi0, PowerTableQi0, Omega, lambda1, lambda2, lambda3);
		#ifdef USE_MPI
		//MpiError = MPI_Barrier(MPI_COMM_WORLD);
		...
		VecGaussIntegrationPhi13_PhiLCBar_PhiLSBar_R23Term(AResultsQi0, BResultsQi0, q.ShortLongr23_r1, q.ShortLongr23_r2Leg, q.ShortLongr23_r2Lag, q.ShortLongr23_r3Leg, q.ShortLongr23_r3Lag, q.ShortLongr23_r12, q.ShortLongr23_phi13, q.ShortLongr23_r23, r2Cusp, r3Cusp, kappa, mu, shpower, sf, NumTermsQi0, PowerTableQi0, Omega, lambda1, lambda2, lambda3);
		//VecGaussIntegrationPhi12_PhiLCBar_PhiLSBar_R23Term(AResultsQi0, BResultsQi0, q.ShortLongr23_r1, q.ShortLongr23_r2Leg, q.ShortLongr23_r2Lag, q.ShortLongr23_r3Leg, q.ShortLongr23_r3Lag, q.ShortLongr23_r12, q.ShortLongr23_phi13, q.ShortLongr23_r23, r2Cusp, r3Cusp, kappa, mu, sf, NumTermsQi0, PowerTableQi0, Omega, Lambda);
		#ifdef USE_MPI
		//MpiError = MPI_Barrier(MPI_COMM_WORLD);
	...
	if (NumTermsQiGt0 > 0) {  // Skips when no terms with qi > 0
		...
		VecGaussIntegrationPhi23_PhiLCBar_PhiLSBar_Full(AResultsQiGt0, BResultsQiGt0, q.ShortLongQiGt0_r1, q.ShortLongQiGt0_r2Leg, q.ShortLongQiGt0_r2Lag, q.ShortLongQiGt0_r3Leg, q.ShortLongQiGt0_r3Lag, q.ShortLongQiGt0_r12,q. ShortLongQiGt0_r13, q.ShortLongQiGt0_phi23, r2Cusp, r3Cusp, kappa, mu, shpower, sf, NumTermsQiGt0, PowerTableQiGt0, Omega, lambda1, lambda2, lambda3);
		#ifdef USE_MPI
