! This program calculates the matrix elements of the positronium-hydrogen D-Wave
...
subroutine WriteHeader(iwrite, Optimize, EigenNum, IsTriplet, Ordering, Method, Omega, Alpha, Beta, Gamma, NumTerms)
	...
	write (iwrite,'(a)') "    <lvalue>2</lvalue>"  ! Indicates D-wave
	if (IsTriplet == 0) then
	...
	write (iwrite,'(a)') "    <gradlapl>Gradient-Gradient</gradlapl>"  ! Only does the grad-grad
	write (iwrite,'(a)') "    <formalism>1</formalism>"  ! Only the first formalism
...
subroutine GenOmegaPowerTable(Omega, PowerTable, NumTerms, Ordering)
	...
	do i = 1, NumTerm, 1
		...
		PowerTable(NumTerm+i,2) = PowerTable(i,2) + 2
		...
		PowerTable(i,1) = PowerTable(i,1) + 2
	end do

!	do i = 1, NumTerm*2, 1
!		write (*,'(i5,a,6i5)') i, ": ", PowerTable(i,1), PowerTable(i,2), PowerTable(i,3), PowerTable(i,4), PowerTable(i,5), PowerTable(i,6)
!	end do
!	write (*,*)
...
! This just cleans up the subroutines in Direct.f90 and Exchange.f90.
...
subroutine CalcMatrices(Omega, NumTerms, Alpha, Beta, Gamma, EigenNum, PhiPhi, Phi2HPhi, &
						qmax, pmax, IsTriplet, Ordering, Method)
	use WCLimits
	...
	lmin = 0;  lmax = 2*(Omega+2) + 2*qmax + 5;
	mmin = -1;  mmax = 2*(Omega+2) + 4;
	nmin = -2 - 2*qmax;  nmax = 2*(Omega+2) + 3;
	...
	if (Method == 0 .or. Method == 1) then
		write (*,*) "Precomputing W and C matrices"
		...
		call CalcWCMatrices(Omega+2, qmax, WMatrix, CMatrix, Alphai+Alphaj, Betai+Betaj, Gammai+Gammaj, pmax)
		write (*,*) "Finished precomputing W matrix"
	else  ! Method == 2 (recursion relations)
		write (*,*) "Recursion relations not currently supported...exiting."
		stop
	...
	!!!write (*,*) "ONLY CALCULATING THE DIRECT TERMS!"
	!!!write (*,*) "!!!"
	!!!goto 300
	...
	! Clean up memory before exiting
300	deallocate(WMatrix, CMatrix)
