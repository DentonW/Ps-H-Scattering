...
real*16 function HylleraasIntegral(RunCalc, j1, j2, j3, j12, j23, j31, alpha, beta, gamma, qmax, Method, WMatrix, CMatrix)
	...
	if (Method == 2) then
		!HylleraasIntegral = HylleraasIntegralRecursion(j1, j2, j3, j12, j23, j31, alpha, beta, gamma, qmax, Method)
		return
	endif
...
real*16 function ModifiedRiemannZeta(N, i, ZetaUpperBound)
	...
	! Thsi is to make it explicitly clear that the hardcoded values are more accurate.
